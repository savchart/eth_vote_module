...
function Body({signer, address}: BodyProps) {
    ...
    const [results, setResults] = useState<{ [key: string]: number }>({ 'Not to be': 0, 'To be': 0 });
    ...
    const castVote = async () => {
        ...
        updateResults(voteFor);
        setShowResults(true);
    }

    const updateResults = (voteFor: number) => {
        const updatedResults = {...results};
        const candidate = candidates[voteFor];
        if (candidate) {
            updatedResults[candidate] = (updatedResults[candidate] || 0) + 1;
        }
        ...
        <Flex height="100vh" width="100vw" alignItems="center" justifyContent="center">
            <Box>
                <InputGroup>
                    ...
                    <Input placeholder='type issue ID number here' value={issue}
                ...
                <Box border={1} borderRadius={5} p={5} m={5}>
                    <RadioGroup onChange={(s) => setVoteFor(parseInt(s))} value={voteFor?.toString()}>
                        <VStack align='start' spacing={4}>
                            {candidates.map((candidate, index) => <Radio key={index}
                                                                         value={index.toString()}>{candidate}</Radio>)}
                        </VStack>
                    </RadioGroup>
                </Box>
                ...
                {showResults && (
                    <Box mt={4}>
                        <Text fontSize="xl">Current Results:</Text>
                        {Object.entries(results).map(([candidate, count]) => (
                            <Text key={candidate}>{candidate}: {count}</Text>
                        ))}
                    </Box>
                )}
            </Box>
